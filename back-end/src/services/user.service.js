...
const queryUsers = async (filter, options) => {
  ...
  const data = await Users.findAndCountAll({ where: filter, limit, offset, order: [[options.sortBy, options.order]], attributes: { exclude: ['password'] } });
  if (!data) {
...
const getTokenByUserIdAndDeviceId = async (id, deviceId) => {
  return Tokens.findOne({ where: { userId: id, deviceId} });
};
 ...
 * @returns {Promise<Tokens>}
 */
const updateRefreshTokenByUserIdAndDeviceId = async (id, deviceId, token, refreshTokenExpires) => {
...
/**
 * Get all tokens by userId
 * @param {Number} id
 * @param {string} deviceId
 * @returns {Promise<Tokens>}
 */
const getAllTokenByUserId= async (id) => {
  // return Tokens.findAll({ where: { userId: id} });
