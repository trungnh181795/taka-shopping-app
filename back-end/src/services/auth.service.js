...
const refreshAuthSQL = async (refreshToken, deviceId) => {
  try {
    const tokenDoc = await tokenService.verifyRefreshTokenSQL(refreshToken, deviceId);
...
const resetPasswordSQL = async (resetPasswordToken, newPassword) => {
  try {
    ...
    await Tokens.destroy({ where: { userID: user.userID, type: tokenTypes.RESET_PASSWORD } });
  } catch (error) {
...
const verifyEmail = async (verifyEmailToken, deviceId) => {
  try {
    const verifyEmailTokenDoc = await tokenService.verifyEmailToken(verifyEmailToken, deviceId);
    const user = await userService.getUserByPk(verifyEmailTokenDoc.userId);
    if (!user) {
      throw new Error();
    }
    await  userService.updateEmailTokenByUserIdAndDeviceId(user.id, deviceId, null);
