 ...
 * @returns {string}
 */
const saveRefreshTokenSQL = async (  userId,
                              token,
                              expires,
                              deviceId) => {
...
const verifyRefreshTokenSQL = async (refreshToken, deviceId) => {
  ...
  const tokenDoc = await Tokens.findOne({ where: { userId, refreshToken, deviceId} });
  if (!tokenDoc) {
...
const verifyEmailToken = async (verifyEmailToken, deviceId) => {
  ...
  const tokenDoc = await Tokens.findOne({ where: { userId, verifyEmailToken, deviceId} });
  if (!tokenDoc) {
    throw new Error('Token not found');
  }
  return tokenDoc;
...
const generateAuthTokens = async (user, deviceId) => {
  ...
  const tokenDoc = await userService.getTokenByUserIdAndDeviceId(user.id, deviceId);
  if (!tokenDoc){
    //create new Token
    await saveRefreshTokenSQL(user.id, refreshToken, refreshTokenExpires, deviceId);
  } else {
    //update Token
    await userService.updateRefreshTokenByUserIdAndDeviceId(user.id, deviceId, refreshToken, refreshTokenExpires);
  }
...
const generateResetPasswordToken = async (userName) => {
  ...
  await saveRefreshTokenSQL(resetPasswordToken, user.userID, expires, tokenTypes.RESET_PASSWORD);
 ...
 * @returns {string}
 */
const saveEmailToken = async (  userId,
                                     token,
                                     expires,
                                     deviceId) => {
  const tokenDoc = await Tokens.create({
...
const generateVerifyEmailToken = async (user, deviceId) => {
  ...
  if (!tokenDoc){
    ...
    //update Token
    await userService.updateEmailTokenByUserIdAndDeviceId(user.id, deviceId, verifyEmailToken);
  }
