...
const login = catchAsync(async (req, res) => {
  ...
  res.send(response(httpStatus.OK, 'Login Success', { user, tokens, deviceId }));
});
...
const refreshTokens = catchAsync(async (req, res) => {
  const tokens = await authService.refreshAuthSQL(req.body.refreshToken, req.body.deviceId);
...
const sendVerificationEmail = catchAsync(async (req, res) => {
  const verifyEmailToken = await tokenService.generateVerifyEmailToken(req.user, req.body.deviceId);
